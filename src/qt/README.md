...
ClientModel is used by the main application `bitcoinrtxgui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/bitcoinrtx/bitcoinrtx/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a bitcoinrtx:... link from a browser).
...
* `bitcoinrtxstrings.cpp`: automatically generated
* `bitcoinrtxunits.(h/cpp)`: BTC / mBTC / etc handling
* `callback.h`
...
* don't change `local/bitcoinrtx_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "bitcoinrtx-qt" under "Run", which is where you can also add command line arguments)
