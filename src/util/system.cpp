...
const char * const BITCOIN_CONF_FILENAME = "bitcoinrtx.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcoinrtxd -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitcoinrtx-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "bitcoinrtx";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinRTX
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinRTX
    // Mac: ~/Library/Application Support/BitcoinRTX
    // Unix: ~/.bitcoinrtx
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinRTX";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinRTX";
#else
    // Unix
    return pathRet / ".bitcoinrtx";
#endif
#endif
}
