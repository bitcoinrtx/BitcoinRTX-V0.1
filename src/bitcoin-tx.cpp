...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " bitcoinrtx-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  bitcoinrtx-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoinrtx transaction\n" +
            "or:     bitcoinrtx-tx [options] -create [commands]   Create hex-encoded bitcoinrtx transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitcoinrtx transaction
