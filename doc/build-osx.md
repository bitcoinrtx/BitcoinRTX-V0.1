...
## Build BitcoinRTX Core

1. Clone the BitcoinRTX Core source code:
    ```shell
    git clone https://github.com/kunalbarchha/bitcoinrtx
    cd bitcoinrtx
    ```

2.  Build BitcoinRTX Core:

    Configure and build the headless BitcoinRTX Core binaries as well as the GUI (if Qt is found).
...
## `disable-wallet` mode
When the intention is to run only a P2P node without a wallet, BitcoinRTX Core may be
...
## Running
BitcoinRTX Core is now available at `./src/bitcoinrtxd`

Before running, you may create an empty configuration file:
```shell
mkdir -p "/Users/${USER}/Library/Application Support/BitcoinRTX"

touch "/Users/${USER}/Library/Application Support/BitcoinRTX/bitcoinrtx.conf"

chmod 600 "/Users/${USER}/Library/Application Support/BitcoinRTX/bitcoinrtx.conf"
```

The first time you run bitcoinrtxd, it will start downloading the blockchain. This process could
...
## Other commands:
```shell
./src/bitcoinrtxd -daemon      # Starts the bitcoinrtx daemon.
./src/bitcoinrtx-cli --help    # Outputs a list of command-line options.
./src/bitcoinrtx-cli help      # Outputs a list of RPC commands when the daemon is running.
```
...
- The Apple keyholder uses this unsigned app to create a detached signature, using the
  script that is also included there. Detached signatures are available from this [repository](https://github.com/kunalbarchha/bitcoinrtx-detached-sigs).
